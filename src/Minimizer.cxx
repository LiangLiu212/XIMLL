...
void fcnMLLGCP(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
		double llf = 0.0;
		...
		pp[6] = (par[6]) * par[7] + (par[6]);
		pp[7] = (par[8]) * par[9] - (par[8]);
		pp[8] = (par[8]) * par[9] + (par[8]);
		pp[9] = (par[6]) * par[7] - (par[6]);
...
double RosenbrockCP(const double * par){
		double pp[10]={0};
		pp[0] = par[0];
		pp[1] = par[1];
...
//=====================================================================
// using minuit2 to minimize the likelihood function
void Minuit2XiXiMLLCP(int index, int MM, TString outfile_name = "out.txt"){
		...
		double xi_phi      = 0.012;   
		double xi_phicp    = 0.0;  
		double L_alpha0     = 0.672;   
		double L_A0cp       = 0.0;   
		double L_alpham     = 0.772;   
		double L_Amcp       = 0.0;
	...
	//	ROOT::Math::Minimizer* min = new ROOT::Minuit2::Minuit2Minimizer(algoName);
		...
		ROOT::Math::Functor f(&RosenbrockCP,10);
		min->SetFunction(f);
		min->SetErrorDef(0.5);
...
void XiXiMLLCP(int index, int MM, TString outfile_name = "out.txt"){
	...
	//	}
		...
		minuit->mnparm(8, "L_alpham" , L_alpham, 0.001, 0., 1., ierflag);
